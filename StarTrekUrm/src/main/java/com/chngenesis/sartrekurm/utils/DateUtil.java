...
public class DateUtil {
	...
	public static Date getTodayZero(){
		return StringToDate(DateToString(new Date(), DateStyle.YYYY_MM_DD), DateStyle.YYYY_MM_DD);
	}
	 ...
	 * @return
	 */
	public static Date getTodayTF(){
		return addDay(getTodayZero(), 1);
	}
	/**
	 * 获取指定时间零时
	 * @return
	 ...
	 * @return
	 */
	public static Date getSpecifiedTF(Date specifiedDate){
		return addDay(StringToDate(DateToString(specifiedDate, DateStyle.YYYY_MM_DD), DateStyle.YYYY_MM_DD), 1);
	}
	/**
	 * 根据当前时间和出生时间计算年龄
	 * 
	...
	public static String getActualAge(Date date, Date birth) {
		int day = calculateIntervalDays(birth, date);
		if(day >= 365) {
			return day / 365 + "岁";
		}
		if(day < 30) {
	 ...
	 * @return
	 */
	/*public static long getSecondBetweenDate(String date,String otherdate){
		Date d = StringToDate(date);
		Date d1 = null;
		if(otherdate == null || "".equals(otherdate)){
			d1 = new Date();
		}
		else{
			d1 = StringToDate(otherdate);
		}
		return d.getTime()-d1.getTime();
	}*/
	
	public static void main(String[] args) {
		System.out.println(DateUtil.addDay(DateUtil.getTodayZero(), 14));
	 ...
	 * @return
	 */
	public static Date[] computationTime(Date startTime,Date endTime,Integer num) {
		if(num == 1) {
			startTime = DateUtil.getZero(new Date());
			endTime =  DateUtil.addDay(startTime, 1);
		}
		if(num == 7) {
			endTime =  DateUtil.addDay(DateUtil.getZero(new Date()), 1);
			startTime = DateUtil.addDay(endTime, -7);
			
		}
		if(num == 15) {
			endTime =  DateUtil.addDay(DateUtil.getZero(new Date()), 1);
			startTime = DateUtil.addDay(endTime, -15);
		}
		if(num == 30) {
			endTime =  DateUtil.addDay(DateUtil.getZero(new Date()), 1);
